...
from models import (
    ...
    Reservation, Transaction, Channel, UserChannelReward, ProviderMessage,
    ServiceGroup, BlockedMessage, AdminAuditLink,
...
def create_main_keyboard(user_id: str = None) -> InlineKeyboardMarkup:
    ...
    try:
        ...
        free_credits_text = "🆓 تجميع رصيد مجاني" if lang_code == 'ar' else "🆓 Free Credits"
...
def create_countries_keyboard(service_id: int, page: int = 0) -> InlineKeyboardMarkup:
    ...
    try:
        ...
        for country in countries:
            ...
            status_text = f"✅ {available_count}" if available_count > 0 else "❌ 0"
            
            keyboard.row(InlineKeyboardButton(
                text=f"{country.flag} {country.country_name} ({status_text})",
        ...
        total_countries = db.query(ServiceCountry).filter(
            ServiceCountry.service_id == service_id,
            ServiceCountry.active == True
        ).count()
        
        if (page + 1) * PAGE_SIZE < total_countries:
...
async def complete_reservation_atomic(reservation_id: int, code: str) -> bool:
    ...
    try:
        ...
        # Calculate price
        price = number.price_override or service.default_price
...
async def process_single_message(provider: Provider, message: Dict[str, Any]):
    ...
    try:
        ...
        await complete_reservation(reservation, code)
...
async def test_group_handler(callback: CallbackQuery):
    ...
    try:
        ...
        try:
            ...
            chat = await bot.get_chat(service_group.group_chat_id)
            
            # Try to get bot member status
            bot_member = await bot.get_chat_member(service_group.group_chat_id, bot.id)
...
async def process_incoming_group_message(message: types.Message):
    ...
    try:
        ...
        # Find matching reservation
        reservation = db.query(Reservation).join(Number).filter(
...
async def start_handler(message: types.Message, state: FSMContext):
    ...
    if is_new_user or not user.language_code:
        ...
        # Show welcome with language selection option (always in Arabic for new users)
        welcome_text = (
        ...
        await message.reply(welcome_text, reply_markup=create_main_keyboard(str(message.from_user.id)))
        return
...
async def language_handler(message: types.Message):
    ...
    keyboard.row(InlineKeyboardButton(text="🔙 العودة", callback_data="main_menu"))
...
async def services_handler(message: types.Message):
    ...
    await message.reply(services_text, reply_markup=create_main_keyboard(str(message.from_user.id)))
...
async def cancel_handler(message: types.Message, state: FSMContext):
    ...
    await message.reply(cancel_text, reply_markup=create_main_keyboard(str(message.from_user.id)))

@dp.message(Command("chatinfo"))
async def chatinfo_handler(message: types.Message):
    """Handle /chatinfo command - useful for getting group ID"""
    chat_info = f"ℹ️ معلومات المحادثة:\n\n"
...
async def free_credits_handler(callback: CallbackQuery):
    """Handle free credits collection"""
    db = get_db()
    try:
        channels = db.query(Channel).filter(Channel.active == True).all()
        
        if not channels:
            await callback.answer("❌ لا توجد قنوات متاحة حالياً")
            return
        ...
        keyboard.row(InlineKeyboardButton(text="✅ تحقق من جميع القنوات", callback_data="verify_all_channels"))
        keyboard.row(InlineKeyboardButton(text="🔙 الرئيسية", callback_data="main_menu"))
...
async def admin_password_handler(message: types.Message, state: FSMContext):
    ...
    if message.text == ADMIN_PASSWORD:
        ...
        await message.reply("❌ كلمة مرور خاطئة")
...
async def ban_user_handler(callback: CallbackQuery):
    ...
    try:
        ...
        # Notify the user
        try:
            await bot.send_message(user.telegram_id, "❌ تم حظرك من استخدام البوت")
        except:
            pass
...
async def unban_user_handler(callback: CallbackQuery):
    ...
    try:
        ...
        # Notify the user
        try:
            await bot.send_message(user.telegram_id, "✅ تم إلغاء حظرك، يمكنك الآن استخدام البوت")
        except:
            pass
...
async def verify_bot_in_group(group_chat_id: str) -> bool:
    ...
    try:
        # Check if bot is admin in the group
        bot_member = await bot.get_chat_member(group_chat_id, bot.id)
